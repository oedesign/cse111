...
# Exceeding requirements: Added known molecules dictionary, get_formula_name function,
# sum_protons function to count total protons, and atomic number in periodic table.
...
def make_periodic_table():
    """Returns a dictionary of the periodic table with element symbols as keys
    and lists of [name, atomic_mass, atomic_number] as values.
    """
    periodic_table_dict = {
        "Ac": ["Actinium", 227, 89],
...
def get_formula_name(formula, known_molecules_dict):
    """Return chemical name or 'unknown compound'."""
    return known_molecules_dict.get(formula, "unknown compound")

def sum_protons(symbol_quantity_list, periodic_table_dict):
    """Compute total number of protons in molecule."""
    total_protons = 0
    for element in symbol_quantity_list:
        symbol = element[SYMBOL_INDEX]
        quantity = element[QUANTITY_INDEX]
...
def main():
    ...
    formula = input("Enter the chemical formula of the molecule: ").strip()
